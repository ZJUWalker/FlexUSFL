...
path = dir + "/server_profile_data.merged.json"
    ...
    for batch in batches[start_batch:]:
        for key, value in batch.items():
            if "timestamp" in key and isinstance(value, list):
                min_time = min(min_time, min(value))
...
for client_id, batches in data.items():
    ...
    for batch in batches[start_batch:]:
        ...
        if "server_fwd_timestamp" in batch:
            start, end = batch["server_fwd_timestamp"]
        ...
        if "server_bwd_timestamp" in batch:
            start, end = batch["server_bwd_timestamp"]
            start_rel = (start - min_time) * 1000
            duration = (end - start) * 1000
            rect = mpatches.Rectangle(
                (start_rel, server_row - row_height / 2),
        ...
        if "server_step_timestamp" in batch:
            start, end = batch["server_step_timestamp"]
            start_rel = (start - min_time) * 1000
            duration = (end - start) * 1000
            rect = mpatches.Rectangle(
                (start_rel, server_row - row_height / 2),
        ...
        if "head_fwd_timestamp" in batch:
            start, end = batch["head_fwd_timestamp"]
            start_rel = (start - min_time) * 1000
            duration = (end - start) * 1000
            rect = mpatches.Rectangle(
        ...
        if "head_bwd_timestamp" in batch:
            start, end = batch["head_bwd_timestamp"]
            start_rel = (start - min_time) * 1000
            duration = (end - start) * 1000
            rect = mpatches.Rectangle(
                (start_rel, client_row - row_height / 2),
        ...
        if "tail_fwd_timestamp" in batch:
            start, end = batch["tail_fwd_timestamp"]
            start_rel = (start - min_time) * 1000
            duration = (end - start) * 1000
            rect = mpatches.Rectangle(
                (start_rel, client_row - row_height / 2),
        ...
        if "tail_bwd_timestamp" in batch:
            start, end = batch["tail_bwd_timestamp"]
            start_rel = (start - min_time) * 1000
            duration = (end - start) * 1000
            rect = mpatches.Rectangle(
                (start_rel, client_row - row_height / 2),
        ...
        if "client_step_timestamp" in batch:
            start, end = batch["client_step_timestamp"]
            start_rel = (start - min_time) * 1000
            duration = (end - start) * 1000
            rect = mpatches.Rectangle(
                (start_rel, client_row - row_height / 2),
...
for client_id, batches in data.items():
    for batch in batches:
        for key, value in batch.items():
            if "timestamp" in key and isinstance(value, list):
                max_time = max(max_time, max(value))
...
plt.tight_layout()
savepath = dir + "/training_timeline.png"
plt.savefig(savepath, dpi=300, bbox_inches="tight")
plt.show()

print("时间线可视化已保存为 training_timeline.png")
