...
class PIQAFedDataset(FedDataset):
    ...
    def _format(self, example):
        ...
        if example['label'] == 0:
            ...
            a = self.a_temp + example["sol2"]
        return {'q': q, 'a': a, 'input': q + '\n' + a}

    def get_answer_mask(self, input_ids, a_ids, tokenizer):
        # 初始化掩码
        answer_mask = torch.zeros_like(input_ids, dtype=torch.int)

        # 遍历每个样本
        for i, (input_seq, a_seq) in enumerate(zip(input_ids, a_ids)):
            # 转换为list方便处理
            input_seq_list = input_seq.tolist()
            a_seq_list = a_seq.tolist()
            # 去除padding后的有效token
            a_seq_list = [token for token in a_seq_list if (token != tokenizer.pad_token_id and token != tokenizer.bos_token_id)]
            # 定位a_seq在input_seq中的起始索引
            for j in range(len(input_seq_list) - len(a_seq_list) + 1):
                if input_seq_list[j : j + len(a_seq_list)] == a_seq_list:
                    answer_mask[i, j : j + len(a_seq_list)] = 1
                    break
    ...
    def _col_fun(self, batch, max_seq_len=-1, extra_info=True):
        ...
        if not extra_info:
            res_dict = {'input_ids': input['input_ids'], 'attention_mask': input['attention_mask'], 'labels': input['input_ids']}
        else:
            res_dict = {
                ...
                'a_text': as_,
                'labels': labels,
                '_mask': self.get_answer_mask(input['input_ids'], input_a['input_ids'], self.tokenizer),
            }
        if self.completion_only:
            res_list_tmp = []
            for i in range(len(batch)):
                res_list_tmp.append(
                    {
                        'input_ids': res_dict['input_ids'][i],
...
class PIQAMiniFedDataset(PIQAFedDataset):
    def _col_fun(self, batch, max_seq_len=-1, **kwargs):
        ...
        input = self.tokenizer(texts, padding=True, truncation=True, return_tensors='pt', max_length=max_len)  # for batch_size testing
...
class STSBFedDataset(FedDataset):
    ...
    def _col_fun(self, batch, max_seq_len=-1, extra_info=True):
        ...
        if not extra_info:
            ...
            res_dict = {'input_ids': input['input_ids'], 'attention_mask': input['attention_mask'], 'input_text': texts, 'labels': labels}
        return res_dict
...
class GSM8KFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(
            tokenizer,
            client_ids,
            load_dataset(env_config.dataset_cache_dir + 'gsm8k', 'main'),
            types=['train', 'test'],
            shrink_frac=shrink_frac,
            **kwargs
        )
    ...
    def _col_fun(self, batch, max_seq_len=-1, **kwargs):
        ...
        # 对整个输入 (question + answer) 分词
        input = self.tokenizer(texts, padding=True, truncation=True, return_tensors='pt', max_length=max_len)

        qs_ = [b['q'] for b in batch]
        as_ = [b['a'] for b in batch]

        # 单独对 question 和 answer 分词
        input_q = self.tokenizer(qs_, padding=True, truncation=True, return_tensors='pt', max_length=max_len)
        input_a = self.tokenizer(as_, padding=True, truncation=True, return_tensors='pt', max_length=max_len)

        # 生成 answer 的掩码
        answer_mask = input_a['attention_mask']  # 每个位置为 1 表示 answer 部分，0 表示填充

        # 如果需要在整个输入中对 answer 生成掩码（假设 question 在前，answer 在后）
        combined_answer_mask = torch.zeros_like(input['attention_mask'])
        for i, (q_len, a_len) in enumerate(zip(input_q['attention_mask'].sum(dim=1), input_a['attention_mask'].sum(dim=1))):
            # question 的长度和 answer 的长度
            q_len = int(q_len.item())  # question 的实际 token 数
            a_len = int(a_len.item())  # answer 的实际 token 数

            # 在合并序列中标记 answer 部分
            combined_answer_mask[i, q_len : q_len + a_len] = 1

        return {
            'input_ids': input['input_ids'],
            'input': input['input_ids'],
            'attention_mask': input['attention_mask'],
            '_mask': combined_answer_mask,  # 返回 answer 的掩码
            'input_text': texts,
            'q_ids': input_q['input_ids'],
            'q_att_mask': input_q['attention_mask'],
            'a_ids': input_a['input_ids'],
...
class DialogSumFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(
            tokenizer, client_ids, load_dataset(env_config.dataset_cache_dir + 'dialogsum'), ['train', 'test', 'validation'], shrink_frac, **kwargs
        )
    ...
    def _col_fun(self, batch, max_seq_len=-1, **kwargs):
        ...
        # 单独对 dialogue 和 summary 分词
        input_q = self.tokenizer(qs_, padding=True, truncation=True, return_tensors='pt', max_length=max_len)
        input_a = self.tokenizer(as_, padding=True, truncation=True, return_tensors='pt', max_length=max_len)

        # 生成 summary 的掩码
        # (1) 单独的 summary 掩码
        summary_mask = input_a['attention_mask']  # 每个位置为 1 表示 summary 部分，0 表示填充

        # (2) 在整个序列中的 summary 掩码
        combined_summary_mask = torch.zeros_like(input['attention_mask'])
        for i, (q_len, a_len) in enumerate(zip(input_q['attention_mask'].sum(dim=1), input_a['attention_mask'].sum(dim=1))):
            q_len = int(q_len.item())  # dialogue 的实际 token 数
            a_len = int(a_len.item())  # summary 的实际 token 数

            # 标记 summary 在合并序列中的位置
            combined_summary_mask[i, q_len : q_len + a_len] = 1
        ...
        return {
            ...
            'q_text': qs_,
            'a_text': as_,
            '_mask': combined_summary_mask,  # 合并序列中的 summary 掩码
        }
...
class CodeAlpacaFedDataset(FedDataset):
    ...
    def _col_fun(self, batch, max_seq_len=-1, **kwargs):
        texts = [b['input'] for b in batch]
        max_len = 400 if max_seq_len < 0 else max_seq_len
        input = self.tokenizer(texts, padding=True, truncation=True, max_length=max_len, return_tensors='pt')  # 400, chatglm256
        qs_ = [b['q'] for b in batch]
        as_ = [b['a'] for b in batch]
        input_q = self.tokenizer(qs_, padding=True, truncation=True, return_tensors='pt', max_length=max_len)
        input_a = self.tokenizer(as_, padding=True, truncation=True, return_tensors='pt', max_length=max_len)

        anwser_mask = input_a['attention_mask']  # 每个位置为 1 表示 answer 部分，0 表示填充
        combined_answer_mask = torch.zeros_like(input['attention_mask'])
        for i, (q_len, a_len) in enumerate(zip(input_q['attention_mask'].sum(dim=1), input_a['attention_mask'].sum(dim=1))):
            q_len = int(q_len.item())  # question 的实际 token 数
            a_len = int(a_len.item())  # answer 的实际 token 数
...
class IMDBFedDataset(FedDataset):
    ...
    def _col_fun(self, batch, max_seq_len=-1, **kwargs):
        ...
        labels = [b['label'] for b in batch]
        labels = torch.tensor(labels)
        return {'input_ids': input['input_ids'], 'attention_mask': input['attention_mask'], 'input_text': texts, 'labels': labels}
...
class WikiTextFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac=0.3, **kwargs):
        dataset = load_dataset(env_config.dataset_cache_dir + 'wikitext', 'wikitext-2-v1')
        types = ['train', 'test', 'validation']
    ...
    def _pre_process(self, ds, batch_size):
        ...
        def group_texts(examples):
            ...
            result["input"] = result["input_ids"].copy()
...
class WikiText103FedDataset(WikiTextFedDataset):
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac=0.3, **kwargs):
        dataset = load_dataset(env_config.dataset_cache_dir + 'wikitext', 'wikitext-103-v1')
        types = ['train', 'test', 'validation']
...
class SensiMarkedFedDataset(FedDataset):
    ...
    def _col_fun(self, batch, max_seq_len=-1, **kwargs):
        texts = [b['input'] for b in batch]
        max_len = 300 if max_seq_len < 0 else max_seq_len
        input = self.tokenizer(texts, padding=True, truncation=True, return_tensors='pt', max_length=max_len)  # 300, ChatgGLM bs=2时降低
        ...
        return {
            ...
            'attention_mask': input['attention_mask'],
            'input_text': texts,
            'entities': [b['entity'] for b in batch],
            'input_santi_mask': mask,
        }

    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        self.df = pd.read_csv(env_config.dataset_cache_dir + 'sensi/sensi.csv')
        dataset = {
...
class ImageWoofFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 1.0):
        ds = load_dataset(env_config.dataset_cache_dir + 'imagewoof', '160px')
    ...
    def _col_fun(self, batch, max_seq_len=-1, **kwargs):
        ...
        return {'input': self.tokenizer(images, return_tensors='pt', padding=True)['pixel_values'], 'labels': labels, 'image': images}
...
class E2EDataset(FedDataset):
    ...
    def _col_fun(self, batch, max_seq_len=-1, extra_info=True):
        ...
        combined_completion_mask = torch.zeros_like(input['attention_mask'])
        for i, (q_len, a_len) in enumerate(zip(input_q['attention_mask'].sum(dim=1), input_a['attention_mask'].sum(dim=1))):
            q_len = int(q_len.item())  # dialogue 的实际 token 数
            a_len = int(a_len.item())  # summary 的实际 token 数

            combined_completion_mask[i, q_len : q_len + a_len] = 1
        return {
            'input_ids': input['input_ids'],
            'input': input['input_ids'],
            'attention_mask': input['attention_mask'],
            ...
            'q_att_mask': input_q['attention_mask'],
            'a_att_mask': input_a['attention_mask'],
            '_mask': combined_completion_mask,
            'q_text': qs_,
            'a_text': as_,
        }
